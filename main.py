...
user_levels = {}
user_history = {}
daily_cooldown = {}  # Lưu mốc thời gian nhận xu hàng ngày của từng user
...
class SlotView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("❌ Máy này người khác đang chơi! Hãy gõ `!quayhu` để mở máy riêng của sếp.", ephemeral=True)
            return False
        return True
    ...
    async def spin(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if balance < self.bet_amount:
            await interaction.response.send_message(f"❌ Không đủ xu! Số dư: **{balance:,} xu**. Chờ ngày mai dùng lệnh `/nhanxu` hoặc xin Admin nạp vốn nhé.", ephemeral=True)
            return
...
async def on_ready():
    try:
        ...
        print(f"Đã đồng bộ thành công {len(synced)} lệnh gạch chéo (Slash Commands).")
...
# ==================== LỆNH THÀNH VIÊN (/nhanxu - GIỚI HẠN 1 LẦN/NGÀY) ====================
...
async def nhanxu(interaction: discord.Interaction):
    ...
    now = datetime.now(timezone(timedelta(hours=7))) # Múi giờ Việt Nam (GMT+7)
    today_str = now.strftime("%Y-%m-%d")

    # Kiểm tra xem hôm nay user đã nhận chưa
    if user_id in daily_cooldown and daily_cooldown[user_id] == today_str:
        await interaction.response.send_message("⏳ Sếp đã điểm danh nhận xu hôm nay rồi! Vui lòng quay lại vào **ngày mai** nhé.", ephemeral=True)
        return

    # Lưu lại mốc đã nhận của ngày hôm nay
...
# ==================== BỘ LỆNH ADMIN PRO (KHÓA CHẶT QUYỀN ADMIN) ====================
...
# Bắt lỗi khi người thường cố tình bấm lệnh Admin Pro
@napxu.error
@congxu.error
@truxu.error
@setxu.error
