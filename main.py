...
@bot.event
async def on_ready():
    print(f"Bot Quay Hũ Pro đã sẵn sàng đầy đủ tính năng nạp xu cho Admin!")
...
# ==================== BỘ LỆNH ADMIN PRO (NẠP / BƠM / TRỪ / SET XU) ====================
@bot.command(name="napxu")
@commands.has_permissions(administrator=True)
async def napxu(ctx, member: discord.Member, amount: int):
    user_id = member.id
    if user_id not in user_balances:
        user_balances[user_id] = 0
    user_balances[user_id] += amount
    await ctx.send(f"💳 [Admin Pro] Nạp thành công **{amount:,} xu** vào tài khoản của **{member.name}**. Số dư mới: **{user_balances[user_id]:,} xu**.")
...
async def congxu(ctx, member: discord.Member, amount: int):
    ...
    if user_id not in user_balances:
        user_balances[user_id] = 0
    user_balances[user_id] += amount
    await ctx.send(f"🛠️ [Admin Pro] Đã thưởng **{amount:,} xu** cho **{member.name}**. Số dư mới: **{user_balances[user_id]:,} xu**.")
...
async def setxu(ctx, member: discord.Member, amount: int):
    ...
    await ctx.send(f"🛠️ [Admin Pro] Đã set số dư của **{member.name}** thành chính xác **{user_balances[user_id]:,} xu**.")
...
async def admin_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        ...
        await ctx.send("⚠️ Sai cú pháp! Ví dụ: `!napxu @tên_user 50000` hoặc `!truxu @tên_user 1000`")
