...
# Hình ảnh minh họa khi quay hũ (sếp có thể thay đổi link ảnh tùy ý)
...
async def quayhu(ctx):
    ...
    # Gửi tin nhắn hiệu ứng quay
    msg = await ctx.send(embed=embed_spin)
    
    # Tạo cảm giác hồi hộp bằng cách đổi khung hình giả lập đang quay
    ...
    is_win = is_jackpot or (slot1 == slot2 or b:=slot2 == slot3 or slot1 == slot3)
    
    # Tạo màu sắc và tiêu đề tùy theo kết quả
    if is_jackpot:
    ...
    # Tạo Embed kết quả hoàn chỉnh có hình ảnh bắt mắt
    ...
    # Cập nhật tin nhắn cũ thành kết quả
    await msg.edit(embed=embed_result)

# Lấy Token từ biến môi trường trên Render / Railway
